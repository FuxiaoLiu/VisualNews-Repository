...
class Encoder_v(nn.Module):
    def __init__(self,
        ...
        self.layers = nn.ModuleList([EncoderLayer_v(hid_dim,
                                                  n_heads,
                                                  pf_dim,
                                                  dropout)
                                     for _ in range(n_layers)])
...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, hid_dim, n_heads, dropout):
        ...
        self.m_k = nn.Parameter(torch.FloatTensor(1, 40, self.hid_dim))
        self.m_v = nn.Parameter(torch.FloatTensor(1, 40, self.hid_dim))

    def forward(self, query, key, value, mask=None):
        query = query.view(query.size(0), -1, 512)
        key = key.view(key.size(0), -1, 512)
        value = value.view(value.size(0), -1, 512)
        ...
        #print('self.m_k:', self.m_k.size())
        #print('self.m_v:', self.m_v.size())
        m_k = np.sqrt(self.hid_dim) * self.m_k.expand(batch_size, 40, self.hid_dim)
        m_v = np.sqrt(40) * self.m_v.expand(batch_size, 40, self.hid_dim)
        #print('m_k:', m_k.size())
        #print('m_v:', m_v.size())
        #print('key:', key.size())
        ...
        #print('m_v:', m_v.size())
        #print('key:', K.size())
        K = torch.cat([K, m_k], dim=1)
        V = torch.cat([V, m_v], dim=1)
        #print('K1:', K.size())
        #print('V1:', V.size())
        ...
        if mask is not None:
            ...
            energy[:, :, :, :nk] = energy[:, :, :, :nk].masked_fill(mask == 0, -np.inf)
...
class Decoder(nn.Module):
    ...
    def forward(self, trg, enc_src, trg_mask, src_mask, imgs, enc_ref, ref_mask):
        ...
        for layer in self.layers:
            trg, attention = layer(trg, enc_src, trg_mask, src_mask, imgs, enc_ref, ref_mask)
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, reference, trg, imgs):
        ...
        imgs = self.encoder_v(imgs)
        # enc_src = [batch size, src len, hid dim]

        output, attention = self.decoder(trg, enc_src, trg_mask, src_mask, imgs, enc_ref, ref_mask)
...
def translate_sentence(model, src, enc_ref, word_map, caplens, imgs, device):
    ...
    for i in range(max_length):
        ...
        with torch.no_grad():
            output, attention = model.decoder(trg_tensor, enc_src, trg_mask, src_mask, imgs, enc_ref, ref_mask)
