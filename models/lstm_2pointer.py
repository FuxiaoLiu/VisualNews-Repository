...
class AttnDecoderRNN(nn.Module):
    ...
    def __init__(self, attention_dim, embed_dim, decoder_dim, vocab_size,vocab_size1, encoder_dim=512, dropout=0.3):
        ...
        self.f81 = nn.Linear(embed_dim + encoder_dim, decoder_dim)
        ...
        self.attention3 = Attention(encoder_dim, decoder_dim, attention_dim)
        ...
        self.p1 = nn.Linear(decoder_dim + decoder_dim + decoder_dim, 1)
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths, encoded_articles, article_lengths, mask, reference, lenre, mask2):
        ...
        hiddens0, _ = self.lstm(embeddings0)
        ...
        hiddens0 = self.dropout(self.relu(self.f81(hiddens0)))
        ...
        for t in range(max(decode_lengths)):
            ...
            guide_reference, distribution3 = self.attention3(hiddens0[:batch_size_t], h1, mask2[:batch_size_t])
            h10 = self.tanh(self.fc1(torch.cat([guide_caption, guide_reference, h1], dim=1)))
            ...
            attn_value1 = torch.zeros([preds.size(0), preds.size(1)]).to(device)
            attn_value1 = attn_value1.scatter_add_(1, index1, distribution3)
            # attn_value = attn_value.scatter_add(1, index, distribution2)
            q = self.sigmoid(
                self.p1(torch.cat([embeddings[:batch_size_t, t, :], guide_reference, h1[:batch_size_t]], dim=1)))


            preds = (1-p-q) * preds + p * attn_value + q * attn_value1
    ...
    def sample1(self, encoder_out, caps, caplens, word_map, encoded_articles, article_lengths, voab1,mask, reference, lenre, mask2):
        ...
        hiddens0 = self.relu(self.f81(hiddens0))
        ...
        for i in range(max_seq_length):
            ...
            guide_reference, distribution3 = self.attention3(hiddens0, h1, mask2)
            h10 = self.tanh(self.fc1(torch.cat([guide_caption, guide_reference, h1], dim=1)))
            ...
            attn_value1 = torch.zeros([scores.size(0), scores.size(1)]).to(device)
            attn_value1 = attn_value1.scatter_add_(1, index1, distribution3)
            # attn_value = attn_value.scatter_add(1, index, distribution2)
            q = self.sigmoid(self.p1(torch.cat([embeddings, guide_reference, h1], dim=1)))


            scores = (1 - p - q) * scores + p * attn_value + q * attn_value1
